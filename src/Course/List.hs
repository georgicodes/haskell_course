...
-- prop> x `headOr` Nil == x
headOr :: a -> List a -> a
headOr myInt Nil = myInt
headOr _ (myList:._) = myList
...
filter2 _ Nil = Nil
filter2 p (head:.tail) =
  let aname = filter p tail
  in if p head then head :. aname else aname
-- in (if p head then head :. aname else id aname)
...
(++) Nil Nil = Nil
(++) Nil (head :. tail) = head :. tail
--(++) (head :. tail) Nil = head :. tail
(++) (head :. tail) listB = head :. (tail ++ listB)
...
flatten' :: List (List a) -> List a
flatten' x = foldRight (++) Nil x -- or remove x's
-- (a :. b :. c :. Nil) :. (d :. e:. f :. Nil) :. (f:. Nil) :. Nil
-- (a :. b :. c :. Nil) ++ (d :. e:. f :. Nil) ++ (f:. Nil) :. Nil
-- a :. b :. c :. d :. e :. f
